...
- Need to verify the full AI PC -> `labos` path: start `.\start_lab_automation.ps1 -Display`, then on `labos` run `mosquitto_sub -h localhost -v -F '%I %t %p' -t 'lab/vision/#'` and `mosquitto_sub -h localhost -v -F '%I %t %p' -t 'lab/control/+/set'`. You should see fresh vision/status/source/heartbeat and debounced `lab/vision/people_count`, with 0 relay `/set` messages from the AI path.
- Need to verify Monitor safely: publish `monitor` to `lab/automation/mode`, watch `lab/automation/#` for intended-state diagnostics, and confirm `lab/control/+/set` remains silent. Return to `manual` after the check.
- Home Assistant People Count cannot be confirmed through the unauthenticated API if it returns `401 Unauthorized`; in that case verify the MQTT source topic `lab/vision/people_count` and visually confirm the HA dashboard from a logged-in browser.
