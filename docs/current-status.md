...
- **End-to-End Count Path Check**: A follow-up live display run observed debounced `lab/vision/people_count` at `stable_count = 4` with `zone_counts = [0,2,2,0,0,0]` during the initial occupied frame. Later Manual/Monitor/Auto logic captures processed a changed live count of 3 as target `TWO_THREE`. Node-RED accepted the count, published intended-state diagnostics in Monitor, and Monitor produced `0` relay `/set` commands.
- **Home Assistant Count Feed**: The MQTT feed used by the HA People Count entity was verified through `lab/vision/people_count`. Direct unauthenticated API read of the HA entity returned `401 Unauthorized`, so dashboard display still needs operator-side visual confirmation, but no conflicting people-count publisher was observed.
...
- **Latest Safe Auto Logic Result**: A short Auto logic check confirmed fresh `mode_state = auto`, `priority_state = PEOPLE_COUNT`, and `intended_state = TWO_THREE`, then returned to `manual`. No relay `/set` messages were captured during that short logic check. This was not a physical production validation.
