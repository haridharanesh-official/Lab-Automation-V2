...
- Current Auto logic is people-count based, not zone based. Node-RED uses `lab/vision/people_count.stable_count` for stages: `0` delayed OFF after 60 continuous seconds, `1-3` LOW_STAGE with relays `2,3,6,7` ON and `4,8` OFF, and `4+` HIGH_STAGE with relays `2,3,4,6,7,8` ON.
